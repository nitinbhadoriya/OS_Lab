...
int main(){
	...
	vector<Process> rdc,rdi;
	while(rdc.empty()==false || rdi.empty()==false || ind!=n){
		while(ind<n && arr[ind].a_time<=t){
			rdc.push_back(arr[ind]);
			ind++;
		}
		if(rdc.empty()==false){
			sort(rdc.begin(),rdc.end(),[](Process p1, Process p2){
				return p1.b[p1.ind]<p2.b[p2.ind] || ((p1.b[p1.ind]==p2.b[p2.ind])&& (p1.a_time<=p2.a_time));
			});
			Process &x=rdc.front();
			...
			for(int i=0;i<time;i++){
				...
				if(rdi.empty()==false){
					Process &y=rdi.front();
					y.b[y.ind]--;
					g_io.push_back(y.pid);
					if(y.b[y.ind]==0){
						y.ind++;
						if(y.ind!=y.len){
							rdc.push_back(rdi.front());
						}
						rdi.erase(rdi.begin());
					}
				}
